...
pub(super) fn first_seen_relabel_assignment(assignment: &[u16]) -> Vec<u16> {
...
pub(super) fn first_seen_relabel_rle(runs: &mut [(u16, u16)]) {
    let mut label_map = HashMap::new();
    let mut label = 0u16;
...
mod tests {
    ...
    #[test]
    fn dense_permutation_duplicate_old_indices_allowed_pinning_current_behavior() {
        // {0 -> 5, 1 -> 5}: produces a non-bijective dense Vec without error.
        let map: HashMap<usize, usize> = [(0, 5), (1, 5)].into_iter().collect();
        assert_eq!(dense_permutation(&map).unwrap(), vec![5, 5]);
    }
    ...
    fn first_seen_relabel_assignment_empty() {
        assert!(first_seen_relabel_assignment(&[]).is_empty());
    }

    #[test]
    fn first_seen_relabel_assignment_all_same() {
        assert_eq!(first_seen_relabel_assignment(&[7, 7, 7]), vec![1, 1, 1]);
    }

    #[test]
    fn first_seen_relabel_assignment_monotonic() {
        assert_eq!(
            first_seen_relabel_assignment(&[2, 3, 4, 5]),
            vec![1, 2, 3, 4]
        );
    }
    ...
    fn first_seen_relabel_assignment_reversed() {
        assert_eq!(
            first_seen_relabel_assignment(&[5, 4, 3, 2]),
            vec![1, 2, 3, 4]
        );
    }
    ...
    fn first_seen_relabel_assignment_with_gaps() {
        assert_eq!(
            first_seen_relabel_assignment(&[1, 5, 9, 5, 1, 9]),
            vec![1, 2, 3, 2, 1, 3]
        );
    }
    ...
    fn first_seen_relabel_rle_basic() {
        ...
        first_seen_relabel_rle(&mut runs);
    ...
    fn first_seen_relabel_assignment_equals_rle_path() {
        ...
        for input in inputs {
            let from_assignment = first_seen_relabel_assignment(&input);
