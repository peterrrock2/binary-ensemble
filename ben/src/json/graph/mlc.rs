...
/// Per-phase progress tracker for MLC, with one spinner line per recursion
/// depth.
///
/// Phase 1 (depth 0) processes the original nodes; phase 2 processes the
/// level-1 clusters produced by phase 1; and so on. Bars are added lazily
/// the first time a given depth is reached, and each bar's total grows as
/// new work at that depth is discovered (e.g. when the next top-level
/// component recurses and introduces more coarse nodes).
///
/// Spinners auto-hide when stderr is not a terminal (e.g. under `cargo
/// test` or when output is piped), so no config is needed for CI/test
/// environments.
struct MlcProgress {
    multi: MultiProgress,
...
impl MlcProgress {
    ...
    /// Make sure a bar exists for `depth`, creating any intermediate bars
...
///
/// Components are sorted by decreasing size (ties broken by minimum label)
/// so that larger components occupy the beginning of the output. Each
/// component is ordered independently by [`mlc_component`].
///
/// # Arguments
///
...
///   seeds and sorting neighbors. Indexed by `NodeIndex::index()`.
/// * `progress` - Progress tracker for the multi-phase spinner display.
/// * `depth` - Recursion depth (0 at the top level). Used to route progress
///   updates to the correct phase bar.
///
/// # Returns
///
...
/// Order a single connected component by seed-expansion clustering plus
...
/// 3. Each cluster is re-ordered internally via [`rcm_component`] on its
///    induced subgraph, so peripheral leaves bracket the cluster and the
///    high-degree seed sits in the interior.
/// 4. If the partition returns a single cluster (a star that covers the
///    whole component), that RCM-ordered cluster is the final order.
/// 5. Otherwise a coarse graph is built with one node per cluster, and
///    [`mlc_order_inner`] recurses on it to decide the order in which
///    clusters are emitted. The recursion terminates when each coarse
///    component collapses to a single cluster.
/// 6. The final order is produced by unrolling: emit clusters in the
///    recursive coarse order, each cluster in its RCM-ordered form.
///
/// # Arguments
///
...
/// * `progress` - Progress tracker for the multi-phase spinner display.
/// * `depth` - Recursion depth; routes progress updates to the correct
///   phase bar.
///
/// # Returns
///
    ...
    Ty: petgraph::EdgeType,
{
    ...
    // `greedy_cluster_partition` ticks this depth's progress per cluster,
    // so every node in `component` contributes to phase `depth+1` exactly
    // once.
    let mut clusters = greedy_cluster_partition(graph, labels, component, progress, depth);

    // Reorder each cluster internally via RCM on the subgraph induced by
    // its members. This puts peripheral (degree-1) nodes at both ends of
    // the cluster and the high-degree seed near the middle/end, which
    // keeps cluster boundaries "loose" and avoids stranding the most-
    // connected node next to the previous cluster.
    ...
    // Multi-cluster case: recurse on the coarse graph to decide the order
    // in which the clusters appear.
...
/// Partition a component into star-shaped clusters using a greedy
/// seed-expansion strategy.
///
/// At each step, the lowest-degree unassigned node (ties broken by label) is
/// chosen as a seed, and the seed together with all of its unassigned
/// neighbors becomes the next cluster. Local degrees are then decremented
/// for every unassigned node adjacent to a newly-assigned one, so subsequent
/// seed selections reflect the residual graph.
///
/// Only cluster *membership* is meaningful here; the internal order of each
/// returned cluster is not final and is expected to be overwritten by the
/// caller (e.g. via [`rcm_component`]).
///
/// # Arguments
///
/// * `graph` - The full graph (only edges within `component` are relevant).
...
/// * `progress` - Progress tracker; `depth`'s done counter is advanced by
///   each cluster's size as the cluster is formed, so the caller's phase
///   bar fills up gradually during large partitions.
/// * `depth` - Recursion depth of the caller, used to select the phase
///   bar to update.
///
/// # Returns
///
    ...
    Ty: petgraph::EdgeType,
{
    ...
    while !remaining.is_empty() {
        ...
        // Cluster membership is seed + every unassigned in-component
        // neighbor. Internal order here is irrelevant: the caller
        // (`mlc_component`) overwrites it with an RCM ordering on the
