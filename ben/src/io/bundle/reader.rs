...
//!
//! - [`BendlReader::asset_bytes`] and [`BendlReader::asset_reader`] are
//!   **verify-on-touch**: the CRC32C of the on-disk payload bytes is
//!   computed as data flows through, and a mismatch is reported at EOF.
//! - [`BendlReader::asset_bytes_unverified`], [`BendlReader::asset_reader_unverified`],
//!   and [`BendlReader::asset_payload_reader_unverified`] are the
//!   explicit recovery/debug escape hatches; they never surface a
//!   [`ChecksumError`].
//! - [`BendlReader::verify_asset_checksum`] and
//!   [`BendlReader::verify_all_asset_checksums`] are explicit raw-bytes
//!   verifiers (no decoding) that do not return decoded payload bytes.
...
impl<R: Read + Seek> BendlReader<R> {
    ...
    /// returning.
    ///
    /// **Contract:** this is exactly `asset_reader(entry)? then read_to_end` — one behavioral
    /// path shared with the streaming API so the two cannot drift apart. Implications:
    ///
    /// - Uncompressed asset, payload byte flipped → the CRC tee observes the mismatch at raw EOF
    ///   and returns
    ///   [`BendlReadError::Checksum`].
    ...
    ///   reaches EOF; surface is [`BendlReadError::Decode`]. (CRC is over compressed bytes, but
    ///   the decoder's failure precedes any CRC check.)
    ...
    /// - Entry has `ASSET_FLAG_CHECKSUM` clear (foreign/hand-built bytes; the library writer
    ///   never produces this) → [`ChecksumError::Unavailable`].
    ...
    /// `io::Error::new(io::ErrorKind::InvalidData, ChecksumError)` on the call that would
    /// otherwise return `Ok(0)` at EOF. Early-drop or partial-read callers do **not** observe
    /// verification — the reader must be driven to EOF for the CRC to be checked.
    pub fn asset_reader<'a>(
        ...
        if entry.asset_flags & ASSET_FLAG_XZ != 0 {
            ...
            // compressed bytes; the BENDL-owned wrapper around the decoder finalizes the
            // check after the codec reaches its own EOF.
    ...
    /// Raw on-disk payload reader without CRC verification — kept
    /// distinct from [`Self::asset_reader_unverified`] so that callers
    /// doing low-level recovery never accidentally emit decompressed
    /// bytes (or, conversely, never accidentally emit compressed bytes
    /// expecting raw).
    ///
    /// For an xz-flagged asset this yields the compressed payload
    /// bytes byte-for-byte; for an uncompressed asset it is the same
    /// as [`Self::asset_reader_unverified`].
    ...
    /// Verify the stored CRC32C of a single asset without returning
    /// any decoded bytes.
    ///
    /// The CRC is over the raw on-disk payload bytes; no decoder is
    /// invoked, so corrupted xz framing under an intact stored CRC
    /// will still report `Ok(())` (or, conversely, an intact xz
    /// payload with a corrupted stored CRC will deterministically
    /// report [`ChecksumError::Mismatch`]).
    pub fn verify_asset_checksum(
        ...
        while remaining > 0 {
            ...
            if n == 0 {
                // Short read against the declared payload length —
                // surface as an I/O error so callers can distinguish a
                // truncated bundle from a CRC mismatch.
    ...
    /// Verify every asset's CRC in directory order. Returns the
    /// **first** mismatch encountered and stops; callers that want a
    /// full audit should iterate the directory and call
    /// [`Self::verify_asset_checksum`] per entry themselves.
    pub fn verify_all_asset_checksums(&mut self) -> Result<(), BendlReadError> {
        // Clone the entries so we don't borrow self.directory across
        // the seek/read calls on self.inner.
...
enum VerifyState {
    ...
    /// (normal EOF).
    EofChecked,
    /// CRC mismatch was reported to the caller. Subsequent reads return `Ok(0)` so the reader
    /// stays well-behaved if the caller re-polls after the error.
    Failed,
}

/// Uncompressed-asset verifying reader: forwards bytes from the bounded payload, accumulates
/// CRC32C as they fly past, and on raw EOF either confirms the checksum or returns
...
/// substitute a checksum error for raw EOF — the codec needs to see the natural `Ok(0)` so it
/// can flush pending output. The post-decoder wrapper ([`DecodedVerifyingReader`])
/// inspects this struct's accumulated hash after codec EOF.
...
/// Verifying wrapper around an `XzDecoder<CrcTeeReader<…>>`. Lets the codec observe normal raw
/// EOF before finalizing the CRC check at the decoded layer.
...
fn classify_read_error(err: io::Error, entry: &BendlDirectoryEntry) -> BendlReadError {
    if err.get_ref().is_some_and(|e| e.is::<ChecksumError>()) {
            ...
            .map(|boxed| boxed.downcast::<ChecksumError>())
        {
            ...
            Some(Err(other)) => {
                // Downcast failed unexpectedly — reconstruct an io::Error
                // around the still-boxed payload so we don't lose context.
