...
//!
//! The authoritative values for `major_version`, `minor_version`,
//! `assignment_format`, `complete`, and the BEN variant all live in the
//! fixed bundle header (or in the embedded stream banner for the variant).
//! The `metadata.json` asset is a best-effort human-readable mirror
//! intended for debugging and tooling; writers should prefer reading the
//! header directly rather than trusting fields in this struct.

use serde::{Deserialize, Serialize};

/// Serde representation of the optional `metadata.json` asset.
///
/// Field names mirror the header where possible so that the JSON is
/// easy to cross-reference against the binary layout.
...
pub struct BendlManifest {
    ...
    /// BEN variant (`"standard"`, `"mkv_chain"`, or `"two_delta"`) as
    /// carried by the embedded stream's 17-byte banner.
