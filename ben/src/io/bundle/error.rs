...
//! Returned `Read` / iterator / stream-wrapper values keep their native `io::Result` surface;
//! checksum failures on those paths are carried as `io::ErrorKind::InvalidData` with an inner
//! [`ChecksumError`] that callers can downcast.
//!
//! Variant discipline is held at the wrap site, not by the type system: `Io(io::Error)` and
//! `Decode(io::Error)` carry the same payload type, so a future refactor could accidentally wrap a
//! decoder-runtime error as `Io(_)` and the type system would not notice. The error-discipline
...
///
/// - **Asset checksum:** `Unavailable` > `Mismatch`. The directory entry is authoritative
///   regardless of bundle finalization, so `verify_asset_checksum` never returns
///   `BundleIncomplete`.
/// - **Stream checksum:** `BundleIncomplete` > `Unavailable` > `Mismatch`. The stream's stored CRC
///   depends on `stream_len` being authoritative, which only holds after finalization, so an
///   unfinalized bundle short-circuits to `BundleIncomplete` before the flag is inspected.
#[derive(Debug, Error)]
pub enum ChecksumError {
    ...
    /// The relevant checksum-presence flag (`ASSET_FLAG_CHECKSUM` on a directory entry, or the
    /// stream-level equivalent on the header) was clear; there is no stored checksum to verify
    /// against. The library writer always sets these flags, so this fires only for foreign or
    /// hand-built bytes.
...
/// High-level error returned by BENDL convenience APIs that consume internally before producing an
/// owned value.
///
/// See [`crate::io::bundle::reader::BendlReader`] for the variant rules per API. The variant
/// discipline is held at the wrap site (where each underlying error becomes a `BendlReadError`);
/// the type system alone cannot prevent a future refactor from mis-wrapping a codec error as `Io`
/// or a header parse failure as `DecoderInit`. The "variant discipline" tests pin which variant
/// fires for each representative read path.
#[derive(Debug, Error)]
pub enum BendlReadError {
    /// Underlying I/O failure at the bundle layer (seek, range read, filesystem). Never used to
    /// carry codec or checksum failures.
    #[error("IO error: {0}")]
    Io(io::Error),

    /// A format-layer error. Reserved for higher-level APIs that wrap an `open` failure or for
    /// future lazy-validation paths; normal post-open accessors should not produce this from
    /// header/directory structure.
    #[error("bundle format error: {0}")]
    Format(BendlFormatError),

    /// Checksum verification failed, was unavailable, or could not be authoritatively performed.
    ...
    /// A codec error raised while a BEN/XBEN/xz decoder was already running (malformed compressed
...
impl From<BendlFormatError> for BendlReadError {
    fn from(e: BendlFormatError) -> Self {
        // BendlFormatError already carries an `Io` arm; unwrap it so that ordinary I/O failures at
        // the format layer surface as `BendlReadError::Io` rather than getting buried inside a
