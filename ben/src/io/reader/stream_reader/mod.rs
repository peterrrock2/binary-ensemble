//! Unified reader for the BEN-stack stream layer (layer 3 — see
//! `docs/glossary.md`).
//!
//! Hides the wire-format choice (BEN bit-packed vs ben32 columnar) and the
//! transport choice (plain vs xz-compressed) behind one type. The decode-side
//! laziness invariant is preserved on both wire formats: frame payload bytes
//! stay opaque until [`crate::codec::BenDecodeFrame::expand`] (frame-level
//! decode), not to be confused with
...
///
/// The Rust representation of the BEN/XBEN stream choice. This is the seam
/// the public reader API uses to dispatch on wire format; the bundle layer
/// owns its own conversion from `AssignmentFormat`.
...
///
/// Construct with [`BenStreamReader::from_ben`] or
/// [`BenStreamReader::from_xben`]. Both arms expose the same downstream
/// surface for assignment iteration, JSONL writing, sample counting, and
/// subsampling.
...
/// Wire-format split: the `Ben` arm carries inline state, the `XBen` arm is
/// boxed so the enum's static size stays close to the smaller plain-BEN
/// footprint instead of being dictated by the larger xz state.
...
impl<R: Read> BenStreamReader<R> {
    ...
    /// Open an XBEN stream. The reader must produce, after xz decompression,
    /// a 17-byte BEN banner followed by ben32 columnar frames.
    ...
    /// Build from a decompressed XBEN stream already positioned past the
    /// 17-byte BEN banner.
    pub(crate) fn from_xben_decompressed(
    ...
    ///
    /// In the `Ben` arm, this clears any active spinner. In the `XBen` arm,
    /// `for_each_assignment` consults `silent` before creating its local
    /// spinner.
    pub fn silent(mut self, silent: bool) -> Self {
        self.silent = silent;
        if let BenStreamInner::Ben {
            spinner, ..
        } = &mut self.inner
        {
            if silent {
    ...
    /// Consume this decoder and iterate over raw BEN/ben32 frames instead of
    ...
    pub fn count_samples(self) -> io::Result<usize> {
        ...
        match self.inner {
            BenStreamInner::Ben { reader, .. } => {
                ben::count_samples_ben(reader, variant)
            }
    ...
    ///
    /// Unlike [`Iterator`], this avoids cloning the assignment buffer on every
    /// frame. The callback receives a borrowed slice and its repetition
    /// count. Return `true` to continue or `false` to stop early.
