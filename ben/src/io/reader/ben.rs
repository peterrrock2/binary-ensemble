...
use crate::codec::{BenDecodeFrame, TwoDeltaFrame};
...
impl<R: Read> BenDecoder<R> {
    ...
    fn pop_standard_frame_from_reader(
        &mut self,
        with_count: bool,
    ) -> Option<io::Result<BenDecodeFrame>> {
        let mut b1 = [0u8; 1];
        let max_val_bit_count = match self.reader.read_exact(&mut b1) {
            Ok(()) => b1[0],
            Err(e) => {
        ...
        Some(Ok(BenDecodeFrame {
            max_val_bit_count,
            max_len_bit_count,
            n_bytes,
            raw_bytes: raw_assignment,
            count,
        }))
    }
...
fn decode_ben_frame_to_assignment(frame: &BenDecodeFrame) -> io::Result<Vec<u16>> {
    decode_ben_line(
...
impl<R: Read> Iterator for BenFrameDecoeder<R> {
    type Item = io::Result<BenDecodeFrame>;
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self.inner.variant {
            ...
            BenVariant::TwoDelta => match self.inner.next() {
                Some(Ok((assignment, count))) => {
                    let encoded = BenDecodeFrame::from_assignment(&assignment);
                    let raw_data = encoded.as_slice()[6..].to_vec();
                    Some(Ok(BenDecodeFrame {
                        max_val_bits: encoded.max_val_bit_count,
                        max_len_bits: encoded.max_len_bit_count,
                        count,
...
///
/// Returns the expanded assignment vector.
fn decode_frame_to_assignment(frame: &DecodeFrame) -> io::Result<Vec<u16>> {
    match frame {
