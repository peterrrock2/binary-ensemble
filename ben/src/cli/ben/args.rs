...
pub(super) struct Args {
    ...
    /// If this flag is set, then this logic is ignored and the output is printed to stdout. This
    /// flag is considered a higher priority than the output_file flag, so if both are present, the
    ...
    /// If input and output files are not provided, then this tells the x-encode, x-decode, and
    /// decode modes that the expected formats are BEN and XBEN
    #[arg(short = 'b', long)]
    pub ben_and_xben: bool,
    /// If input and output files are not provided, then this tells the x-encode and x-decode modes
    /// that the expected formats are JSONL and XBEN
    #[arg(short = 'J', long)]
    pub jsonl_and_xben: bool,
    /// If the input and output files are not provided, then this tells the decode mode that the
    /// expected formats are JSONL and BEN
    #[arg(short = 'j', long)]
    pub jsonl_and_ben: bool,
    /// When saving a file in the BEN format, the deault is to have an assignment vector saved
    /// followed by the number of repetitions of that assignment vector (this is useful for Markov
    /// chian methods like ReCom). This flag will cause the program to forgo the repetition count
    /// and just save all of the assignment vectors as they are encountered. Equivalent to
    ...
    #[arg(short = 'q', long)]
    pub quiet: bool,
    /// Number of threads the XZ encoder may use during x-encode and xz-compress. Defaults to 1
    /// (single-threaded). Pass an explicit value to fan compression out across worker threads;
    /// values larger than the host's available parallelism are silently clamped down. `-1` is a
    /// sentinel meaning "use every available core" (sklearn convention). See also
    /// `--xz-block-size`, which controls how much input each thread gets before it can start
    /// compressing.
    #[arg(short = 'c', long, allow_hyphen_values = true)]
    pub n_cpus: Option<i32>,
    /// When running x-encoder, this flag will deterimine the level of compression to use. By
    /// default, the highest level of compression will be used. Valid values are 0-9, where 0 is no
    /// compression and 9 is the highest level of compression.
    #[arg(short = 'l', long)]
    pub compression_level: Option<u32>,
    /// Number of TwoDelta delta frames per columnar chunk in XBEN encoding. Only affects TwoDelta
    /// variant. Larger chunks improve XZ compression. Default is 10,000.
    #[arg(long)]
    ...
    /// bundle. The graph is added after the assignment stream has been fully written. Only applies
    /// to the `encode` and `x-encode` modes.
    #[arg(long)]
