...
pub(in crate::cli::ben) fn run(args: LookupArgs, g: &Globals) -> CliResult {
    ...
    let (input_file, index) = match args.target {
        LookupTarget::Index(args) => (args.input_file, args.index),
    };
    ...
    let vec = {
        ...
        extract_assignment_ben_seek(reader, index).map_err(|e| CliError::other(format!("{e}")))?
    };
