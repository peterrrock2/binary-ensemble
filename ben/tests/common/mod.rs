...
//!
//! Each integration test crate declares `mod common;` to opt in. The
//! module re-exports the in-crate test utilities from
//! `binary_ensemble::test_utils` and adds integration-only helpers
...
///
/// Returns the same `env!("CARGO_BIN_EXE_*")` value the existing test
/// helpers use; centralised here so future CLI tests can pick up the
