...
| {mod}`binary_ensemble.codec`  | `ben encode` / `decode`     | Whole-file conversions between JSONL, BEN, and XBEN |
| {mod}`binary_ensemble.graph`  | `ben relabel` / `sort-graph` | Reordering a dual graph before encoding             |
...
**`stream`** — when you specifically _don't_ want a bundle: a raw `.ben`/`.xben` stream with
