...
| Look up one sample       | `decoder.lookup(index)`                 | Zero-based, like `ben lookup FILE INDEX`; on `BenDecoder` and `BendlDecoder`                                                                                                                                     |
...
| Compact a bundle         | automatic                               | `remove_asset`, `compress_stream`, `decompress_stream`, and `relabel_bundle` emit compact bundles (appends leave only a superseded directory behind, a few bytes each); `bendl compact` reclaims dead space in files from other tools |
...
| Decompress bundle stream | `decompress_stream(...)`                | Embedded XBEN stream to embedded BEN stream                                                                                                                                                                      |
