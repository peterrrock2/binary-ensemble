...
However, before we can see the gains that relabeling buy us, we will need to
...
format. In our running example, if we then compress this file back
...
However, this is not the only thing that we can do to make the file smaller.
...
This is where the other aspects of `reben`com into play. If we would like to produce
...
* The maximum value of any assignment value is assumed to be 65535 (the
...
This, as the name Lempel-Ziv Markov Algorithm would suggest, is particularly good
at improving the compression ensembles generated by Markov methods like 
GerryChain or Forest Recom, but it is also good generally at improving the
compression of general BEN files. The LZMA2 algorithm is based off of the
[LZ-77](https://en.wikipedia.org/wiki/LZ77_and_LZ78#LZ77) algorithm which
replaces repeated occurrences of data with a single copy of that data 
appearing earlier in the data stream. LZMA2 then uses a Markov process to dynamically
determine the best Variable-Length Encoding (VLE) that can be assigned to the
most frequently occurring sequential bytes of data that appear within
a data stream, and then encodes the data using these codes (for a simplified
idea of what is going on here, see [Huffman coding](https://en.wikipedia.org/wiki/Huffman_coding)).
...
converting from a BEN file to an XBEN file, we actually unpack each assignment
vector into an intermediate format known as BEN32, which is an RLE that uses
32 bits to encode each assignment vector (using a big-endian formatted u16 for
the value and a big-endian formatted u16 for the length and a null u32 as the
separator). The LZMA2 algorithm is then able to make use of the repetition
...
CLI tool has been build with an emphasis on fast decompression, so any
...
big of an issue, so it is worth keeping that in mind.
...
(e.g. Recom or Forest Recom), then the [PCompress](https://github.com/mggg/pcompress)
